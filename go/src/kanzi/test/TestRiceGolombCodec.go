...
func main() {
	fmt.Printf("\nTestRiceGolombCodec")
	var outputName = flag.String("output", "TestRiceGolombCodec.log", "optional name of the output file (defaults to TestRiceGolombCodec.log)")
	flag.Parse()
...
func TestCorrectness(filename string) {
	...
	for ii := 1; ii < 20; ii++ {
		...
		dbgbs.ShowByte(true)
		dbgbs.Mark(true)
		fpc, _ := entropy.NewRiceGolombEncoder(dbgbs, true, 4)
		...
		fpd, _ := entropy.NewRiceGolombDecoder(dbgbs2, true, 4)
