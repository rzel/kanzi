...
public final class DCT16 implements IntTransform
{
    ...
    {
       this.computeForward(block, blkptr, this.data, 0, 6);
       this.computeForward(this.data, 0, block, blkptr, this.fShift-6);
       return block;
    }
    ...
    {       
       ...
       for (int i=0; i<16; i++)
       {
          ...
          output[j]     = ((W0  *c0)  + (W1  *c1)  + round) >> shift;
          output[j+16]  = ((W16 *a2)  + (W17 *a3)  + (W18 *a6)  + (W19 *a7)  + 
                           (W20 *a10) + (W21 *a11) + (W22 *a14) + (W23 *a15) + round) >> shift;
          output[j+32]  = ((W32 *b2)  + (W33 *b3)  + (W34 *b6)  + (W35 *b7)  + round) >> shift;
          output[j+48]  = ((W48 *a2)  + (W49 *a3)  + (W50 *a6)  + (W51 *a7)  + 
                           (W52 *a10) + (W53 *a11) + (W54 *a14) + (W55 *a15) + round) >> shift;
          output[j+64]  = ((W64 *c2)  + (W65 *c3)  + round) >> shift;
          output[j+80]  = ((W80 *a2)  + (W81 *a3)  + (W82 *a6)  + (W83 *a7)  + 
                           (W84 *a10) + (W85 *a11) + (W86 *a14) + (W87 *a15) + round) >> shift;
          output[j+96]  = ((W96 *b2)  + (W97 *b3)  + (W98 *b6)  + (W99 *b7)  + round) >> shift;
          output[j+112] = ((W112*a2)  + (W113*a3)  + (W114*a6)  + (W115*a7)  + 
                           (W116*a10) + (W117*a11) + (W118*a14) + (W119*a15) + round) >> shift;
          output[j+128] = ((W128*c0)  + (W129*c1)  + round) >> shift;
          output[j+144] = ((W144*a2)  + (W145*a3)  + (W146*a6)  + (W147*a7)  + 
                           (W148*a10) + (W149*a11) + (W150*a14) + (W151*a15) + round) >> shift;
          output[j+160] = ((W160*b2)  + (W161*b3)  + (W162*b6)  + (W163*b7)  + round) >> shift;
          output[j+176] = ((W176*a2)  + (W177*a3)  + (W178*a6)  + (W179*a7)  + 
                           (W180*a10) + (W181*a11) + (W182*a14) + (W183*a15) + round) >> shift;
          output[j+192] = ((W192*c2)  + (W193*c3)  + round) >> shift;
          output[j+208] = ((W208*a2)  + (W209*a3)  + (W210*a6)  + (W211*a7)  + 
                           (W212*a10) + (W213*a11) + (W214*a14) + (W215*a15) + round) >> shift;
          output[j+224] = ((W224*b2)  + (W225*b3)  + (W226*b6)  + (W227*b7)  + round) >> shift;
          output[j+240] = ((W240*a2)  + (W241*a3)  + (W242*a6)  + (W243*a7)  + 
    ...
    private int[] computeInverse(int[] input, int iIdx, int[] output, int oIdx, int shift)
    {
       ...
       for (int i=0; i<16; i++)
       {
          ...
          final int x1  = input[j+16];
          final int x2  = input[j+32];
          final int x3  = input[j+48];
          final int x4  = input[j+64];
          final int x5  = input[j+80];
          final int x6  = input[j+96];
          final int x7  = input[j+112];
          final int x8  = input[j+128];
          final int x9  = input[j+144];
          final int x10 = input[j+160];
          final int x11 = input[j+176];
          final int x12 = input[j+192];
          final int x13 = input[j+208];
          final int x14 = input[j+224];
          final int x15 = input[j+240];
          ...
          output[offs]    = (d0  >= MAX_VAL) ? MAX_VAL : ((d0  <= MIN_VAL) ? MIN_VAL : d0);
          output[offs+1]  = (d1  >= MAX_VAL) ? MAX_VAL : ((d1  <= MIN_VAL) ? MIN_VAL : d1);
          output[offs+2]  = (d2  >= MAX_VAL) ? MAX_VAL : ((d2  <= MIN_VAL) ? MIN_VAL : d2);
          output[offs+3]  = (d3  >= MAX_VAL) ? MAX_VAL : ((d3  <= MIN_VAL) ? MIN_VAL : d3);
          output[offs+4]  = (d4  >= MAX_VAL) ? MAX_VAL : ((d4  <= MIN_VAL) ? MIN_VAL : d4);
          output[offs+5]  = (d5  >= MAX_VAL) ? MAX_VAL : ((d5  <= MIN_VAL) ? MIN_VAL : d5);
          output[offs+6]  = (d6  >= MAX_VAL) ? MAX_VAL : ((d6  <= MIN_VAL) ? MIN_VAL : d6);
          output[offs+7]  = (d7  >= MAX_VAL) ? MAX_VAL : ((d7  <= MIN_VAL) ? MIN_VAL : d7);
          output[offs+8]  = (d8  >= MAX_VAL) ? MAX_VAL : ((d8  <= MIN_VAL) ? MIN_VAL : d8);
          output[offs+9]  = (d9  >= MAX_VAL) ? MAX_VAL : ((d9  <= MIN_VAL) ? MIN_VAL : d9);
          output[offs+10] = (d10 >= MAX_VAL) ? MAX_VAL : ((d10 <= MIN_VAL) ? MIN_VAL : d10);
          output[offs+11] = (d11 >= MAX_VAL) ? MAX_VAL : ((d11 <= MIN_VAL) ? MIN_VAL : d11);
          output[offs+12] = (d12 >= MAX_VAL) ? MAX_VAL : ((d12 <= MIN_VAL) ? MIN_VAL : d12);
          output[offs+13] = (d13 >= MAX_VAL) ? MAX_VAL : ((d13 <= MIN_VAL) ? MIN_VAL : d13);
          output[offs+14] = (d14 >= MAX_VAL) ? MAX_VAL : ((d14 <= MIN_VAL) ? MIN_VAL : d14);
          output[offs+15] = (d15 >= MAX_VAL) ? MAX_VAL : ((d15 <= MIN_VAL) ? MIN_VAL : d15);
